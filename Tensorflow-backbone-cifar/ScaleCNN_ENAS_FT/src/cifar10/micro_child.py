...
from src.utils import get_train_ops, get_grads, get_optimization_op
from src.common_ops import create_weight

create_weight = functools.partial(create_weight, enable_dropout=True)
...
class MicroChild(Model):
  ...
  def _factorized_reduction(self, x, out_filters, stride, is_training):
    ...
    if stride == 1:
      with tf.variable_scope("path_conv"):
        inp_c = self._get_C(x)
        w, p = create_weight("w", [1, 1, inp_c, out_filters])
        if self.is_cdropout:
          x = self._concrete_dropout(inputs=x, p=p, data_format=self.actual_data_format)
    ...
    with tf.variable_scope("path1_conv"):
      ...
      w, p = create_weight("w", [1, 1, inp_c, out_filters // 2])
      if self.is_cdropout:
        path1 = self._concrete_dropout(inputs=path1, p=p, data_format=self.actual_data_format)
    ...
    with tf.variable_scope("path2_conv"):
      ...
      w, p = create_weight("w", [1, 1, inp_c, out_filters // 2])
      if self.is_cdropout:
        path2 = self._concrete_dropout(inputs=path2, p=p, data_format=self.actual_data_format)
  ...
  def _maybe_calibrate_size(self, layers, out_filters, is_training):
    ...
    with tf.variable_scope("calibrate"):
      ...
      if hw[0] != hw[1]:
        ...
        with tf.variable_scope("pool_x"):
          w, p = create_weight("w", [1, 1, c[0], out_filters])
          x = tf.nn.relu(x)
          if self.is_cdropout:
            x = self._concrete_dropout(inputs=x, p=p, data_format=self.actual_data_format)
          x = tf.nn.conv2d(x, w, [1, 1, 1, 1], "SAME",
      ...
      if c[1] != out_filters:
        with tf.variable_scope("pool_y"):
          w, p = create_weight("w", [1, 1, c[1], out_filters])
          y = tf.nn.relu(y)
          if self.is_cdropout:
            y = self._concrete_dropout(inputs=y, p=p, data_format=self.actual_data_format)
  ...
  def _model(self, images, is_training, reuse=False):
    ...
    with tf.variable_scope(self.name, reuse=reuse):
      ...
      with tf.variable_scope("stem_conv"):
        w, p = create_weight("w", [3, 3, 3, self.out_filters * 3])
        if self.is_cdropout:
          images = self._concrete_dropout(inputs=images, p = p, data_format=self.actual_data_format)
        x = tf.nn.conv2d(
        ...
        if (self.use_aux_heads and
          ...
          with tf.variable_scope("aux_head"):
            ...
            with tf.variable_scope("proj"):
              ...
              w, p = create_weight("w", [1, 1, inp_c, 128])
              if self.is_cdropout:
                aux_logits = self._concrete_dropout(inputs=aux_logits, p=p, data_format=self.actual_data_format)
            ...
            with tf.variable_scope("avg_pool"):
              ...
              w, p = create_weight("w", [hw, hw, inp_c, 768])
              if self.is_cdropout:
                aux_logits = self._concrete_dropout(inputs=aux_logits, p=p, data_format=self.actual_data_format)
              aux_logits = tf.nn.conv2d(aux_logits, w, [1, 1, 1, 1], "SAME",
                                        data_format=self.data_format)
            ...
            with tf.variable_scope("fc"):
              ...
              w, p = create_weight("w", [inp_c, 10])
              if self.is_cdropout:
                aux_logits = self._concrete_dropout(inputs=aux_logits, p=p, data_format='fc')
      ...
      if is_training and self.keep_prob is not None and self.keep_prob < 1.0 and not self.is_cdropout:
        x = tf.nn.dropout(x, self.keep_prob)
      with tf.variable_scope("fc"):
        inp_c = self._get_C(x)
        w, p = create_weight("w", [inp_c, 10])
        if self.is_cdropout:
          x = self._concrete_dropout(inputs=x, p=p, data_format='fc')
        x = tf.matmul(x, w)
    return x
  ...
  def _fixed_conv(self, x, f_size, out_filters, stride, is_training,
    ...
    for conv_id in range(stack_convs):
      ...
      with tf.variable_scope("sep_conv_{}".format(conv_id)):
        w_depthwise, _ = create_weight("w_depth", [f_size, f_size, inp_c, 1], enable_dropout=False)
        w_pointwise, p_pointwise = create_weight("w_point", [1, 1, inp_c, out_filters])
        x = tf.nn.relu(x)
        ...
        if self.is_cdropout:
          x = self._concrete_dropout(inputs=x, p=p_pointwise, data_format=self.actual_data_format)
        x = tf.nn.conv2d(x, w_pointwise, [1, 1, 1, 1], "SAME",
                       data_format=self.data_format)
        '''
      ...
      with tf.variable_scope("cell_{}".format(cell_id)):
        ...
        with tf.variable_scope("x_conv"):
          ...
          if (x_op in [0, 1, 2, 3] and
              ...
              is_training): #and not self.is_cdropout):
  ...
  def _build_train(self):
    ...
    if self.num_gpus > -1:
      self.optimizer, self.lr = get_optimization_op(
        self.global_step,
      ...
      with tf.variable_scope(tf.get_variable_scope()) as _:
        for gpu_index in range(0, self.num_gpus):
          with tf.device('/gpu:%d' % gpu_index):
            with tf.name_scope('%s_%d' % ('gpu', gpu_index)) as scope:
              print("-" * 80)
              print("Build train graph on gpu_"+str(gpu_index))

              x_train, y_train = self.train_batch_queue.dequeue()
              logits = self._model(x_train, is_training=True)
              ...
              if self.use_aux_heads:
                log_probs = tf.nn.sparse_softmax_cross_entropy_with_logits(
                  logits=self.aux_logits, labels=y_train)
              ...
              self.train_acc = tf.equal(self.train_preds, y_train)
              ...
              grads, grad_norm, grad_norms = get_grads(
                self.optimizer,
                train_loss,
                tf_variables,
                ...
                l2_reg=self.l2_reg,
                device_scope='%s_%d'%('gpu', gpu_index))
              tower_grads.append(grads)
              tower_grad_norm.append(grad_norm)
              #tower_grad_norms.append(grad_norms)
        ...
        #tower_grad_norms = _average_gradients(tower_grad_norms)

      self.train_op = self.optimizer.apply_gradients(tower_grads, global_step=self.global_step)
    else:
      print("-" * 80)
  ...
  def _build_valid(self):
    if self.x_valid is not None:
      if self.num_gpus < -1:
        ...
        with tf.device('/gpu:%d' % (self.num_gpus)):
          with tf.name_scope('%s_%d' % ('gpu_', self.num_gpus)) as scope:
            print("-" * 80)
            print("Build valid graph")
            logits = self._model(self.x_valid, False, reuse=True)
            self.valid_preds = tf.argmax(logits, axis=1)
  ...
  def connect_controller(self, controller_model, selection_mode=False):
    if self.fixed_arc is None:
