...
class Model(object):
    def __init__(self,
        ...
        self.num_gpus_for_train = self.num_gpus if self.num_gpus>1 else 1
        ...
        with tf.device("/cpu:0"):
            ...
            x_train, y_train = tf.train.shuffle_batch(
                ...
                batch_size=self.batch_size//self.num_gpus_for_train,
                capacity=50000,
                enqueue_many=True,
                min_after_dequeue=0,
                num_threads=16,
            ...
            if self.data_format == "NCHW":
                self.x_train.set_shape((self.batch_size//self.num_gpus_for_train, 3, 32, 32))
            else:
                self.x_train.set_shape((self.batch_size//self.num_gpus_for_train, 32, 32, 3))
            self.y_train = y_train
            self.y_train.set_shape((self.batch_size//self.num_gpus_for_train,))
            self.train_batch_queue = tf.contrib.slim.prefetch_queue.prefetch_queue([self.x_train, self.y_train],
                                                                                   capacity=10 * self.num_gpus_for_train)
    ...
    def _concrete_dropout(self, inputs, p, data_format, determinist=True):
        ...
        if data_format == 'channels_first':
            ...
            noise_shape = (tf.shape(inputs)[0], input_shape[1], 1, 1)
        elif data_format == 'channels_last':
            assert len(input_shape) == 4
            noise_shape = (tf.shape(inputs)[0], 1, 1, input_shape[3])
        else:
            assert len(input_shape) == 2
            noise_shape = (tf.shape(inputs)[0], input_shape[1])
        '''
        if determinist:
            print('Using determinist drop.')
            """
            unif_noise_1_var = tf.get_variable('unif_noise_1',
                                               initializer=tf.random_uniform(shape=[1]+list(noise_shape[1:])), trainable=False)
            unif_noise_2_var = tf.get_variable('unif_noise_2',
                                               initializer=tf.random_uniform(shape=[1]+list(noise_shape[1:])), trainable=False)
            """
            unif_noise_var = tf.get_variable('unif_noise',
                                             initializer=tf.random_uniform(shape=[1] + list(noise_shape[1:])),
                                             trainable=False)
            drop_tensor = tf.math.floor(unif_noise_var + p)
            random_tensor = 1. - drop_tensor
        else:
            unif_noise_1_var = tf.random_uniform(shape=noise_shape)
            unif_noise_2_var = tf.random_uniform(shape=noise_shape)

            drop_prob = (
                tf.log(p + eps)
                - tf.log(1. - p + eps)
                + tf.log(-tf.log(unif_noise_1_var + eps) + eps)
                - tf.log(-tf.log(unif_noise_2_var + eps) + eps)
            )
        ...
        '''
        inputs = tf.nn.dropout(x=inputs, keep_prob=0.7, noise_shape=noise_shape)
