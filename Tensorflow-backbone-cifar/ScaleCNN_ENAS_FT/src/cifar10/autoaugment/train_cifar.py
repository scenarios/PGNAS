...
import src.cifar10.autoaugment.custom_ops as ops
import src.cifar10.autoaugment.data_utils as data_utils
import src.cifar10.autoaugment.helper_utils as helper_utils
import numpy as np
from src.cifar10.autoaugment.shake_drop import build_shake_drop_model
from src.cifar10.autoaugment.shake_shake import build_shake_shake_model
import tensorflow as tf
from src.cifar10.autoaugment.wrn import build_wrn_model
...
tf.flags.DEFINE_float("weight_regularizer_ratio", 100.0, "Weight regularizer_ratio for Concrete Dropout")
tf.flags.DEFINE_float("dropout_regularizer_ratio", 1.0, "Dropout regularizer_ratio for Concrete Dropout")
...
class CifarModel(object):
  ...
  def _build_graph(self, images, labels, mode):
    ...
    # Adds KL regularization to the cost
    #print("Using KL regularization")
    #self.cost = helper_utils.KL(self.cost)
    ...
    with tf.device('/cpu:0'):
      self.saver = tf.train.Saver(max_to_keep=5, var_list=[v for v in tf.global_variables() if 'global_step' not in v.name])
      self.restore_saver = tf.train.Saver(max_to_keep=2,
                                          var_list=[v for v in tf.global_variables()
                                                    if 'noise' not in v.name and 'global_step' not in v.name])
...
class CifarModelTrainer(object):
  ...
  def extract_model_spec(self):
    ...
    if checkpoint_path is not None:
      self.restore_saver.restore(self.session, checkpoint_path)
      tf.logging.info('Loaded child model checkpoint from %s',
                      checkpoint_path)
    else:
      self.save_model(step=0)

  def extract_model_spec_init(self):
    """Loads a checkpoint with the architecture structure stored in the name."""
    checkpoint_path = tf.train.latest_checkpoint(self.model_dir)
    if checkpoint_path is not None:
      self.restore_saver.restore(self.session, checkpoint_path)
  ...
  def eval_child_model(self, model, data_loader, mode, init=False):
    ...
    while True:
      try:
        if init:
          with self._new_session_init(model):
  ...
  def _build_models(self):
    ...
    with tf.variable_scope('model', use_resource=False):
      ...
      self._saver = m.saver
      self._restore_saver = m.restore_saver
      ...
      try:
        if curr_epoch > 0:
          with self._new_session(m):
            ...
            tf.logging.info('Saving model after epoch, not using init session')
            self.save_model(step=curr_epoch)
            break
        else:
          with self._new_session_init(m):
            train_accuracy = helper_utils.run_epoch_training(
              self.session, m, self.data_loader, curr_epoch)
            tf.logging.info('Saving model after epoch, using init session')
            self.save_model(step=curr_epoch)
            break
  ...
  def run_model(self):
    ...
    with tf.Graph().as_default(), tf.device(
      ...
      valid_accuracy = self.eval_child_model(
        meval, self.data_loader, 'test', init=True)
      tf.logging.info('Before Training Epoch: {}     Test Acc: {}'.format(
  ...
  def run_selection(self):
    """Trains and evalutes the image model."""
    for run_id in range(1500):
      acc_file = open(os.path.join(FLAGS.checkpoint_dir, 'acc_recorder.txt'), 'a')
      tf.reset_default_graph()
      g = tf.Graph()
      with g.as_default(), tf.device(
            '/cpu:0' if FLAGS.use_cpu else '/gpu:0'):
        # Build the child graph
        tf.set_random_seed(run_id + 1)
        ...
        test_accuracy = self.eval_child_model(
          meval, self.data_loader, 'test')
        tf.logging.info('Before Training Epoch: {}     Test Acc: {}'.format(
          starting_epoch, test_accuracy))
      acc_file.write(str(test_accuracy) + '\n')
      acc_file.close()
  @property
  def saver(self):
    return self._saver
...
def main(_):
  ...
  global_args.WEIGHT_REGULARIZER_RATIO[0] = FLAGS.weight_regularizer_ratio
  global_args.DROPOUT_REGULARIZER_RATIO[0] = FLAGS.dropout_regularizer_ratio
  ...
  if FLAGS.model_name == 'wrn':
    ...
    hparams.add_hparam('lr', 0.01)
    hparams.add_hparam('weight_decay_rate', 0.001)
  elif FLAGS.model_name == 'shake_shake_160':
    hparams.add_hparam('model_name', 'shake_shake')
    hparams.add_hparam('num_epochs', 1800)
    hparams.add_hparam('shake_shake_widen_factor', 10)
    hparams.add_hparam('lr', 0.01)
    hparams.add_hparam('weight_decay_rate', 0.001)
