...
def _shake_shake_skip_connection(x, output_filters, stride):
  ...
  path1 = ops.conv2d_with_concrete_dropout(path1, int(output_filters / 2), 1, scope='path1_conv')
  ...
  path2 = ops.conv2d_with_concrete_dropout(path2, int(output_filters / 2), 1, scope='path2_conv')
...
def _shake_shake_branch(x, output_filters, stride, rand_forward, rand_backward,
  ...
  x = tf.nn.relu(x)
  x = ops.conv2d_with_concrete_dropout(x, output_filters, 3, stride=stride, scope='conv1')
  x = ops.batch_norm(x, scope='bn1')
  x = tf.nn.relu(x)
  x = ops.conv2d_with_concrete_dropout(x, output_filters, 3, scope='conv2')
...
def build_shake_shake_model(images, num_classes, hparams, is_training):
  ...
  x = ops.conv2d_with_concrete_dropout(x, 16, 3, scope='init_conv')
  ...
  # Fully connected
  logits = ops.fc_with_concrete_dropout(x, num_classes)
  return logits
