...
from src import utils, global_args
...
DEFINE_boolean("is_cdropout", False, "To use Concrete Dropout or not.")


DEFINE_integer("log_every", 50, "How many steps to log")
DEFINE_integer("eval_every_epochs", 1, "How many epochs to eval")

DEFINE_float("weight_regularizer_ratio", 100.0, "Weight regularizer_ratio for Concrete Dropout")
DEFINE_float("dropout_regularizer_ratio", 2.0, "Dropout regularizer_ratio for Concrete Dropout")
DEFINE_integer("num_gpus", 1, "Number of gpus used for training.")


def get_ops(images, labels, selection_mode=False):
    """
    Args:
    ...
    if FLAGS.child_fixed_arc is None:
        ...
        child_model.connect_controller(None, selection_mode=True)
        controller_ops = None

    child_ops = None if selection_mode else {
...
def train():
    ...
    with g.as_default():
        ...
        partial_saver_1 = tf.train.Saver(max_to_keep=2, var_list=[v for v in tf.global_variables() if 'global' not in v.name and 'T_i' not in v.name and 'Momentum' not in v.name and 'last_reset' not in v.name])
        partial_saver_2 = tf.train.Saver(max_to_keep=2, var_list=[v for v in tf.global_variables() if 'moving' in v.name])
        ...
        def partial_init(scaffold, sess):
            partial_saver_1.restore(sess, os.path.join(FLAGS.output_dir, 'restore/pretrained.ckpt'))
            #partial_saver_2.restore(sess, os.path.join(FLAGS.output_dir, 'restore/model.ckpt-157500'))

        with tf.train.SingularMonitoredSession(
                config=config, hooks=hooks, checkpoint_dir=FLAGS.output_dir,
                scaffold=monitored_session.Scaffold(init_op=tf.global_variables_initializer(), saver=saver, init_fn=partial_init)) as sess:
...
def selection():
    for run_id in range(1500):
        acc_file = open(os.path.join(FLAGS.output_dir,'acc_recorder.txt'), 'a')
        ...
        with g.as_default():
            ...
            ops = get_ops(images, labels, selection_mode=True)
            child_ops = ops["child"]
            controller_ops = ops["controller"]

            saver = tf.train.Saver(max_to_keep=2, var_list=[v for v in tf.global_variables() if 'unif_noise' not in v.name])
            selected_saver = tf.train.Saver(max_to_keep=2)
            print("-" * 80)
            print("Starting session")
            config = tf.ConfigProto(allow_soft_placement=True)
            with tf.Session(config=config) as sess:
                ...
                saver.restore(sess, os.path.join(FLAGS.output_dir, 'model.ckpt-219240'))
                print("Round {}: Eval".format(run_id))
                acc = ops["eval_func"](sess, "test")
                print(' ')                
                
                #selected_saver.save(sess, os.path.join(FLAGS.output_dir, 'selected.ckpt'))
                coord.request_stop()
                coord.join(threads)
...
def main(_):
    ...
    global_args.WEIGHT_REGULARIZER_RATIO[0] = flags.FLAGS.weight_regularizer_ratio
    global_args.DROPOUT_REGULARIZER_RATIO[0] = flags.FLAGS.dropout_regularizer_ratio
    print("-" * 80)
