...
class Logger(object):
    def __init__(self, output_file):
        ...
        self.log = open(output_file, "a") if 'hdfs' not in output_file else None
...
def get_train_ops(
    ...
    if l2_reg > 0:
        ...
        print('#' * 80)
        print("Using Concrete Dropout weight regularizer.")
        for dp in tf.get_collection('dropout_p'):
            tf.summary.scalar(dp.name, dp[0])
        KL_loss = tf.add_n(tf.get_collection(key='concrete_dropout_weight_regularizer'))
...
def get_grads(
    ...
    if l2_reg > 0:
        ...
        for dp in tf.get_collection('dropout_p'):
            tf.summary.scalar(dp.name, dp[0])
        KL_loss = tf.add_n(tf.get_collection(key='concrete_dropout_weight_regularizer', scope=device_scope))
        tf.summary.scalar('KL_loss', KL_loss)
        loss += KL_loss

    grads_and_vars = opt.compute_gradients(loss, tf_variables, colocate_gradients_with_ops=True)
    grads = [v[0] for v in grads_and_vars]
    ...
    if clip_mode is not None:
      ...
      if clip_mode == "global":
        ...
        raise NotImplementedError("Unknown clip_mode {}".format(clip_mode))
      '''
    return grads_and_vars, grad_norm, grad_norms


def get_optimization_op(
        train_step,
        lr_warmup_val=None,
        lr_warmup_steps=100,
        lr_init=0.1,
