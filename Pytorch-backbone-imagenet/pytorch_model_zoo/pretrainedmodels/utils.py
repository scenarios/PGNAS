...
class PixelJitter(object):
    ...
    def __init__(self, range=20.0/255.0):
        self.range=range

    def __call__(self, tensor):
        return torch.clamp(
            input=tensor + torch.zeros_like(tensor).detach().uniform_(-self.range, self.range),
            min=0.0, max=1.0
        )
...
class TransformImage(object):
    ...
    def __init__(self, opts, scale=0.875, random_crop=False,
                 ...
                 preserve_aspect_ratio=True, rotate=False, pixel_jitter=False):
        if type(opts) == dict:
            opts = munchify(opts)
        ...
        if rotate:
            tfs.append(transforms.RandomRotation(10))

        if random_crop:
            tfs.append(transforms.RandomResizedCrop(max(self.input_size)))
        else:
        ...
        if pixel_jitter:
            #tfs.append(PixelJitter(range=20.0/255.0))
            tfs.append(transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5))
